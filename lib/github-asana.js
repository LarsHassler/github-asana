...
var processed_commits = {};
var users = [
  { username: 'Lars Haßler', key: '__key_here__'}
];
...
function getTaskActions(commits) {
  ...
  for (var i in commits) {
    var commit_id = commits[i].id.slice(0,7);
    if (commit_id in processed_commits){
      continue;
    }
    processed_commits[commit_id] = 1;
...
function sendTaskCommentsToAsana(tasks) {
  ...
  users.foreach(function(user) {
     auths[user.username] = 'Basic ' + new Buffer(user.key + ":").toString('base64');
  });
  ...
  for (var i in tasks) {
    ...
    if (task.verb=='fix') {
        request.put({
            ...
            headers: {"Authorization": auths[task.author]}
          }, apiCallback);
